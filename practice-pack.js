...
(() => {
  ...
  function initNotes(){
    const noteBtn=$('#notesButton'),drawer=$('#notesDrawer'),close=$('#notesClose'),selectionBtn=$('#selectionNote'),form=$('#noteForm'),quoteEl=$('#noteQuote'),comment=$('#noteComment'),save=$('#noteSave'),cancel=$('#noteCancel'),list=$('#notesList'); if(!noteBtn||!drawer)return;
    const key=`toefl26-notes:${location.pathname}`; let notes=getJSON(key,[]), draft=null, color='yellow', editingId=null;
    const colors=['yellow','mint','blue','pink'];
    function syncColor(next){color=next||'yellow';$$('.color-row button').forEach(x=>x.classList.toggle('active',x.dataset.color===color));}
    ...
    function openExistingNote(note){if(!note)return;editingId=note.id;draft={scopeId:note.scopeId,start:note.start,end:note.end,quote:note.quote};quoteEl.textContent=`“${note.quote}”`;comment.value=note.comment||'';syncColor(note.color);form.classList.remove('hidden');open();requestAnimationFrame(()=>comment.focus());}
    noteBtn.onclick=open;close.onclick=shut;cancel.onclick=resetForm;
    $$('.color-row button').forEach(b=>b.onclick=()=>syncColor(b.dataset.color));
    document.addEventListener('mouseup',()=>{setTimeout(()=>{const sel=getSelection();if(!sel||sel.isCollapsed){selectionBtn.style.display='none';return;}const range=sel.getRangeAt(0);const node=range.commonAncestorContainer.nodeType===1?range.commonAncestorContainer:range.commonAncestorContainer.parentElement;const scope=node.closest?.('[data-note-scope]');if(!scope){selectionBtn.style.display='none';return;}const quote=sel.toString().trim();if(!quote||quote.length<2){selectionBtn.style.display='none';return;}const offsets=rangeOffsets(scope,range); if(!offsets)return; const rect=range.getBoundingClientRect();draft={scopeId:scope.dataset.noteScope,start:offsets.start,end:offsets.end,quote};selectionBtn.style.left=`${Math.min(innerWidth-150,Math.max(8,rect.left))}px`;selectionBtn.style.top=`${Math.max(8,rect.bottom+7)}px`;selectionBtn.style.display='block';},0)});
    ...
    save.onclick=()=>{if(!draft)return;const now=new Date().toISOString();if(editingId!==null){notes=notes.map(n=>String(n.id)===String(editingId)?{...n,comment:comment.value.trim(),color,updatedAt:now}:n);}else{notes.push({...draft,id:Date.now(),comment:comment.value.trim(),color,createdAt:now,updatedAt:now});}setJSON(key,notes);resetForm();renderList();applyHighlights();};
    function renderList(){list.innerHTML=notes.length?notes.map(n=>`<article class="saved-note ${String(editingId)===String(n.id)?'is-active':''}" data-open-note="${n.id}" title="Open this comment"><blockquote>${esc(n.quote)}</blockquote>${n.comment?`<p>${esc(n.comment)}</p>`:'<p>No comment yet — click to add one.</p>'}<footer><button data-edit-note="${n.id}">Open comment</button><button data-del-note="${n.id}">Delete</button></footer></article>`).join(''):'<p style="color:#888c98;font-size:10px;line-height:1.6">Select text in any prompt, then choose “Highlight + comment.” After saving, click the highlighted text itself to reopen its comment.</p>';$$('[data-open-note]',list).forEach(card=>card.onclick=e=>{if(e.target.closest('[data-del-note]'))return;openExistingNote(notes.find(n=>String(n.id)===card.dataset.openNote));});$$('[data-edit-note]',list).forEach(b=>b.onclick=e=>{e.stopPropagation();openExistingNote(notes.find(n=>String(n.id)===b.dataset.editNote));});$$('[data-del-note]',list).forEach(b=>b.onclick=e=>{e.stopPropagation();notes=notes.filter(n=>String(n.id)!==b.dataset.delNote);if(String(editingId)===b.dataset.delNote)resetForm();setJSON(key,notes);renderList();applyHighlights();});}
    ...
    document.addEventListener('click',e=>{if(e.target.closest?.('.notes-drawer,.notes-button,.selection-note,button,input,textarea,select,a,audio'))return;const scope=e.target.closest?.('[data-note-scope]');if(!scope)return;const offset=offsetAtPoint(scope,e.clientX,e.clientY);if(offset===null)return;const hits=notes.filter(n=>n.scopeId===scope.dataset.noteScope&&offset>=n.start&&offset<=n.end).sort((a,b)=>(a.end-a.start)-(b.end-b.start));if(hits.length)openExistingNote(hits[0]);});
    function applyHighlights(){if(!window.CSS?.highlights||typeof Highlight==='undefined')return;for(const c of colors){const ranges=notes.filter(n=>n.color===c).map(n=>{const scope=document.querySelector(`[data-note-scope="${CSS.escape(n.scopeId)}"]`);return scope?rangeFromOffsets(scope,n.start,n.end):null}).filter(Boolean);CSS.highlights.set(`pack-${c}`,new Highlight(...ranges));}}
